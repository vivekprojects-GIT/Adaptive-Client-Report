"""Signals -> rewards -> the answer bandit, and the preference profile.
...
all three:
  ...
  D2 arm     — thumbs on an ANSWER. Rewards the answer strategy that wrote
               ...
               This is the only bandit left in the system.
  engagement — how much of the REPORT the client used. Opened it, stayed
               with it, talked to it, said it helped. Accrues on the report
               row, and reaches the composer as evidence in the skill
               brief. No arm is paid: nothing selects a template.
...
═══════════════════════════════════════════════════════════════════════════
ENGAGEMENT IS CAPPED PER REPORT
═══════════════════════════════════════════════════════════════════════════

Engagement accumulates into at most 1.0 per report, tracked on the Report
row. Without the cap a chatty client would score their report as
world-beating by asking eleven questions — volume of clicks masquerading
as quality of document. The cap mattered when it fed a bandit and still
matters now: the skill brief reads these scores to say which layouts held
this client's attention.
"""
...
# ---------------------------------------------------------------------------
# Event vocabulary. Weights are each event's engagement contribution.
# ---------------------------------------------------------------------------
...
VALID_EVENTS = set(ENGAGEMENT_WEIGHTS) | {
    "report_unhelpful", "block_highlighted", "answer_helpful",
    "answer_unhelpful", "section_viewed",
    # Deliberately absent from ENGAGEMENT_WEIGHTS: asking to see a chart
    # says a great deal about how this client wants to be shown things and
    # nothing about how much of the report they used. It is also absent
    # from _dims_from_event
...
def record_event(session: Session, client_id: str, event_type: str,
    ...
    if (event_type in ENGAGEMENT_WEIGHTS or event_type == "report_unhelpful")             and report_id:
        out["engagement"] = _accrue_engagement(session, report_id, event_type)
...
# ---------------------------------------------------------------------------
# Engagement — capped accrual against the report itself
# ---------------------------------------------------------------------------

def _accrue_engagement(session: Session, report_id: str, event_type: str) -> Dict:
    """How much of this report the client actually used, 0.0 to 1.0.

    Kept after the template bandit was removed, because the score was
    never really about the arm: it is a fact about the report, and
    skill.py reads it to tell the composer which layouts held this
    client's attention. What went with the bandit is the write to the arm
    row — there is no arm, and paying one whose selection count nothing
    increments would make its mean climb on reports it never produced.
    """
    ...
    if event_type == "report_unhelpful":
        ...
        return {"applied": True, "verdict": "negative",
                ...
                "note": "accrual closed — engagement after 'this was "
                        "confusing' must not read as approval"}
    ...
    weight = ENGAGEMENT_WEIGHTS.get(event_type, 0.0)
    ...
    return {"applied": True, "scope": "report",
            "accrued": rep.normalized_reward,
            "note": "engagement recorded against the report; nothing "
                    "selects a template, so there is no arm to reward"}
